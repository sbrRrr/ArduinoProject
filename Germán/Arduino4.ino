...
void setup() {
  ...
  while (!client.connected()) {
    if (client.connect("ArduinoClient4")) { //Cambiar el número de cliente 
...
void loop() {
  ...
  client.publish("arduino4/mediciones/distance", distanceStr);
  client.publish("arduino4/mediciones/speed", speedStr);
