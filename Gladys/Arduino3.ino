...
const char* mqtt_server = "Dirección.broker.MQTT"; // Dirección del broker MQTT=ID de la red
...
void setup() {
  ...
  while (!client.connected()) {
    if (client.connect("ArduinoClient3")) { //Cambiar el número de cliente 
...
void loop() {
  ...
  client.publish("arduino3/mediciones/distance", distanceStr);
  client.publish("arduino3/mediciones/speed", speedStr);
