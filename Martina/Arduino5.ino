...
void setup() {
  ...
  while (!client.connected()) {
    if (client.connect("ArduinoClient5")) { //Cambiar el número de cliente 
...
void loop() {
  ...
  client.publish("arduino5/mediciones/distance", distanceStr);
  client.publish("arduino5/mediciones/speed", speedStr);
